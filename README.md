...
- **Note on model selection:** The initial submission — which briefly held first place on the leaderboard — used **GPT-OSS 120B**. In current pipline when used **gemini 3.1 Flash lite** showed macro f1 of 0.53 (single vote, with out ensemble). That means **GPT -OSS 120B** will surely show a score around 0.5X but the wait time when calling api each time makes experimentation and test run (1200 documents) difficult. The final and best-performing submission switched to **Gemini 3.1 Pro**, with many new additional ideas, a natural evolution of first solution.
